...
void parc::set_uniforms(std::unique_ptr<givr::program> const &p) const {
    set_phong_uniforms(*this, p);
}
...
std::string parc::get_vertex_shader_source() const {
    return std::string(R"shader(
        attribute vec3 position;
        attribute vec3 normal;
        attribute vec2 uvs;
        ...
        void main(){
            ...
            vec4 model_vert = mv * vec4(position, 1.0);
            original_pos = vec3(mv);
            frag_normal = vec3(vec4(normal, 0));
            frag_colour = colour;
        }
...
std::string parc::get_fragment_shader_source() const {
    return std::string(R"shader(
        uniform vec3 colour;
        uniform bool per_vertex_colour;
        uniform vec3 light_position;
